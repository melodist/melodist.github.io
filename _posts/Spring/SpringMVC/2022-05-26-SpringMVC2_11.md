...
업로드하는 HTML Form의 name에 맞추어 `@RequestParam`을 적용. 추가로 `@ModelAttribute`에서도 `MultipartFile`을 동일하게 사용 가능
...
고객이 업로드한 파일명으로 서버 내부에 파일을 저장하면 안됨. 왜냐하면 서로 다른 고객이 같은 파일이름을 업로드 하는 경우 기존 파일 이름과 충돌 가능성 존재. 서버에서는 저장할 파일명이 겹치지 않도록 내부에서 관리하는 별도의 파일명이 필요
...
`createStoreFileName()`: 서버 내부에서 관리하는 파일명은 유일한 이름을 생성하는 UUID 를 사용해서 충돌하지 않도록 함
`extractExt()`: 확장자를 별도로 추출해서 서버 내부에서 관리하는 파일명에도 붙여줌. 예를 들어서 고객이 `a.png`라는 이름으로 업로드 하면 `51041c62-86e4-4274-801d-614a7d994edb.png`와 같이 저장
...
`MultipartFile attachFile`: 멀티파트는 `@ModelAttribute`에서 사용 가능
...
- `@GetMapping("/attach/{itemId}")`: 파일을 다운로드 할 때 실행. 예제를 더 단순화 할 수 있지만, 파일 다운로드 시 권한 체크같은 복잡한 상황까지 가정한다 생각하고 이미지 id 를 요청. 파일 다운로드시에는 고객이 업로드한 파일 이름으로 다운로드 하는 게 좋음. 이때는 `Content-Disposition` 헤더에 `attachment; filename="업로드 파일명"` 값을 줌
...
다중 파일 업로드를 하려면 `multiple="multiple"` 옵션을 주면 됨
`ItemForm` 의 다음 코드에서 여러 이미지 파일 다운로드가 가능
`private List<MultipartFile> imageFiles;`
